...
export type TutorialCategory = 'getting_started' | 'daily_tasks' | 'projects' | 'reports' | 'advanced' | 'troubleshooting';
...
export interface TutorialAnalytics {
  ...
  dropOffPoints: {               // Where users typically stop
    stepNumber: number;
...
export interface TutorialValidationError {
  field: string;
  message: string;
  code: 'REQUIRED' | 'INVALID_FORMAT' | 'TOO_LONG' | 'TOO_SHORT' | 'INVALID_FILE';
}

export interface VideoValidationError {
  field: string;
  message: string;
  code: 'FILE_TOO_LARGE' | 'INVALID_FORMAT' | 'DURATION_TOO_LONG' | 'UPLOAD_FAILED';
}
...
export interface TutorialNotification {
  id: string;
  type: 'new_tutorial' | 'tutorial_updated' | 'completion_reminder' | 'required_tutorial';
  tutorialId: string;
  tutorialTitle: string;
  message: string;
  isRead: boolean;
...
export const isTutorialCategory = (category: string): category is TutorialCategory => {
  return ['getting_started', 'daily_tasks', 'projects', 'reports', 'advanced', 'troubleshooting'].includes(category);
};
...
// Utility functions
export const getTutorialCategoryInfo = (category: TutorialCategory): TutorialCategoryInfo | undefined => {
